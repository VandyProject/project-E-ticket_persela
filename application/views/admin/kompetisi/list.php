...
<script type="text/javascript">
    ...
    $(document).ready(function() {
        ...
        table = $('.dataTable').DataTable({
            ...
            "ajax": {
                "url": "<?php echo site_url('admin/kompetisi/listdata') ?>",
                "type": "POST"
            },
    ...
    function editdata(id)
    {
        ...
        $.ajax({
            url: "<?php echo site_url('admin/kompetisi/edit') ?>/" + id,
            type: "GET",
            dataType: "JSON",
            success: function(data)
            {
                ...
                $('#tambah').modal('show'); // show bootstrap modal
                $('.modal-title').text('Edit Kompetisi Pertandingan'); // Set Title to Bootstrap modal title
                $('#btnSave').text('Update');
                $('#photo-preview').show(); // hide photo preview modal
                if (data.logo_kompetisi)
                {
                    $('#label-photo').text('Change Logo'); // label photo upload
                    $('#photo-preview div').html('<img src="' + base_url + 'logo/' + data.logo_kompetisi + '" class="img-fluid">'); // show photo
                } else
                {
                    $('#label-photo').text('Upload Logo'); // label photo upload
    ...
    function save()
    {
        ...
        if (save_method == 'add') {
            url = "<?php echo site_url('admin/kompetisi/save') ?>";
        } else {
            url = "<?php echo site_url('admin/kompetisi/update') ?>";
        }
    ...
    function deletedata(id)
    {
            ...
            $.ajax({
                url: "<?php echo site_url('admin/kompetisi/hapus') ?>/" + id,
                type: "POST",
                dataType: "JSON",
                success: function(data)
                {
        ...
        <div class="modal-content">
            ...
            <div class="modal-body">
                <form class="form-material" id="form" enctype="multipart/form-data" method="POST" action="">
                    <input type="hidden" name="id_kompetisi" value="">
                        ...
                        <div class="col-sm-12">
                            <div class="material-group material-group-danger">
                                ...
                                <div class="form-group form-danger form-static-label">
                                    <input class="form-control" name="nama_kompetisi" type="text" value="">
                                    <span class="form-bar"></span>
                                    <label class="float-label">Nama Kompetisi</label>
                                </div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="form-group row">
                        <div class="col-sm-12">
                            <div class="material-group material-group-danger">
                                ...
                                <div class="form-group form-danger form-static-label">
                                    <input class="form-control" id="fileup" style="display: none" name="logo_kompetisi" type="file" onchange="document.getElementById('namafile').value=this.value">
