...
<script type="text/javascript">
    ...
    $(document).ready(function() {
        ...
        table = $('.dataTable').DataTable({
            ...
            "ajax": {
                "url": "<?php echo site_url('admin/sponsor/listdata') ?>",
                "type": "POST"
            },
    ...
    function add()
    {
        ...
        $('.modal-title').text('Tambah Sponsor'); // Set Title to Bootstrap modal title
        $('#btnSave').text('Simpan');
    ...
    function editdata(id)
    {
        ...
        $.ajax({
            url: "<?php echo site_url('admin/sponsor/edit') ?>/" + id,
            type: "GET",
            dataType: "JSON",
            success: function(data)
            {

                $('[name="id_sponsor"]').val(data.id_sponsor);
                $('[name="created"]').val(data.created);
                $('[name="user"]').val(data.user);
                $('[name="link_sponsor"]').val(data.link_sponsor);
                $('#tambah').modal('show'); // show bootstrap modal
                $('.modal-title').text('Edit Sponsor'); // Set Title to Bootstrap modal title
                $('#btnSave').text('Update');
                $('#photo-preview').show(); // hide photo preview modal
                if (data.logo_sponsor)
                {
                    $('#label-photo').text('Change Logo'); // label photo upload
                    $('#photo-preview div').html('<img src="' + base_url + 'logo/' + data.logo_sponsor + '" class="img-fluid">'); // show photo
                } else
                {
                    $('#label-photo').text('Upload Logo'); // label photo upload
    ...
    function save()
    {
        ...
        if (save_method == 'add') {
            url = "<?php echo site_url('admin/sponsor/save') ?>";
        } else {
            url = "<?php echo site_url('admin/sponsor/update') ?>";
        }
        ...
        <div class="modal-content">
            ...
            <div class="modal-body">
                <form class="form-material" id="form" enctype="multipart/form-data" method="POST" action="">
                    <input type="hidden" name="id_sponsor" value="">
                        ...
                        <div class="col-sm-12">
                            <div class="material-group material-group-danger">
                                ...
                                <div class="form-group form-danger form-static-label">
                                    <input name="link_sponsor" class="form-control" type="text" value="" placeholder="Link Web Sponsor">
                                    <span class="form-bar"></span>
                                    <label class="float-label">Link Web Sponsor</label>
                                </div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="form-group row">
                        <div class="col-sm-12">
                            <div class="material-group material-group-danger">
                                ...
                                <div class="form-group form-danger form-static-label">
                                    <input class="form-control" id="fileup" style="display: none" name="logo_sponsor" type="file" onchange="document.getElementById('namafile').value=this.value">
