...
class User extends CI_Controller {
    ...
    function index() {
        $this->load->view('template/template', [
            'content' => $this->load->view('managemen/user/list', [
            ], true)
        ]);
    }
    ...
    public function listdata() {
        ...
        $list = $this->crud->getfind_data('ms_user', $this->column_order, $this->column_search, $this->order, 'hak_akses', 'Admin');
        $data = array();
        $no = $_POST['start'];
        $n=0;
        foreach ($list as $l) {
        ...
        $output = array(
            "draw" => $_POST['draw'],
            "recordsTotal" => $this->db->where('hak_akses', 'Admin')->from('ms_user')->count_all_results(),
            "recordsFiltered" => $this->db->where('hak_akses', 'Admin')->get('ms_user')->num_rows(),
            "data" => $data,
        );
